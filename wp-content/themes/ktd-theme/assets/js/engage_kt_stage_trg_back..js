                ...
                onLoggedIn: function(eventData) {
                    ...
                    if (! document.cookie.match(/^(.*;)?\s*auth0_spajs\s*=\s*[^;]+(.*)?$/ || document.cookie.match(/^(.*;)?\s*auth0_sub\s*=\s*[^;]+(.*)?$/))) {
                        if (localStorage.getItem('@@auth0spajs@@::Og9eZ12aZ5kjqIdo8LFUrmzcqilVqKeE::default::openid profile email') != null) {
								...
								document.cookie = "auth0_sub=" + spajsval + ";secure; domain=" + location.hostname + "; path=/; max-age=" + 45*24*60*60;
                        }    
                    }
...
if (! document.cookie.match(/^(.*;)?\s*auth0_sub\s*=\s*[^;]+(.*)?$/))  {
  if (document.cookie.match(/^(.*;)?\s*auth0_spajs\s*=\s*[^;]+(.*)?$/)) {
    document.cookie = "auth0_spajs=;secure; domain=" + location.hostname + "; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/"; 
  }
} else {
    ...
    if (document.cookie.match(/^(.*;)?\s*auth0_spajs\s*=\s*[^;]+(.*)?$/)){
        ...
        document.cookie = "auth0_spajs=" + auth0_spajs_cookie_data + ";secure; domain=" + location.hostname + "; path=/; max-age=" + 45*24*60*60;
    } else {
        if (document.cookie.match(/^(.*;)?\s*auth0_sub\s*=\s*[^;]+(.*)?$/)){
            //document.cookie = "auth0_sub=;secure; domain=" + location.hostname + "; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/"; 
			get_auth_user(getCookie("auth0_sub"));
        }        
    }
}
if (document.cookie.match(/^(.*;)?\s*auth0spajs\s*=\s*[^;]+(.*)?$/)){
    document.cookie = "auth0spajs=;secure; domain=." + 
    location.hostname.split('.').reverse()[1] + "." + 
    location.hostname.split('.').reverse()[0] + "; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/"; 

    document.cookie = "auth0spajs=;secure; domain=" + location.hostname + "; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/"; 
...
function get_auth_user(user_id)
{
   ...
   data: {user_id:readCookie('auth0_sub')},
