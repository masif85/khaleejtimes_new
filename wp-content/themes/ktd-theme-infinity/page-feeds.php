...
if ($feedtype == 'quintype') {
    $template = "quintype-";
}
if ($feedtype == 'multilingual') {
    $template = "multilingual-";
}
if ($feedtype == 'botim') {
    $template = "botim-";
}
if ($feedtype == 'devarabic') {
    $template = "devarabic-";
}
...
//$datas=(array)$articles[0]['objectUpdated'];
...
if (count($articles) > 0) {
   // echo "<pre>";
  // print_r($articles[0]['body_raw']);
    $articleBody = $articles[0]->getParsedBody();
    $articleBodyPresentation = new ArticleBodyPresentation($articles[0]);
    $articleBody->generateContent($articleBodyPresentation);
...
function GetUuidFromOCList($PList, $PLimit) {
    ...
    $query = QueryBuilder::where('', $PList);
    ...
    if ($list ) {
        foreach ($list[0]['articleuuids'] as $item) {

            $ArticlesFromList.=  $item." ";
            $ListCounter++;
