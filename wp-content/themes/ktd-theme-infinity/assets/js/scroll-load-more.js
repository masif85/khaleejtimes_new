...
window.scrollTracker = {
    ...
    //if (window.location.href.indexOf("/arabic/") > -1 || window.location.href.indexOf("=ar") > -1 || window.location.href.indexOf("/ar/") > -1) {        
       ...
       // }
     // var url=string.substr(string.indexOf(".com/") + 5);         
         // url = 'https://site.everywarestarterkit.local/'+url;
    ...
    function loadArticle(trigger) {
      ...
      $.ajax({
        ...
        type: 'POST',
        dataType: 'html',
        url: 'https://api.khaleejtimes.com/home/get_contents?age='+start,
        data: { "url" : infiniteArticles[start-1] + '?infinite=1&skip=' + start},
        success: function(response) {
          try{
          $('.loaded-articles').append(response);
          var string=infiniteArticles[start-1];
          var url=string.substr(string.indexOf(".com/") + 5);         
          url = 'https://site.everywarestarterkit.local/'+url;
        //  window.history.replaceState(null, null, url);
        }
